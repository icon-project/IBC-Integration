...
use crate::rlp::{error::DecoderError, impls::decode_usize, traits::Decodable};
...
impl PayloadInfo {
    ...
    pub fn from(header_bytes: &[u8]) -> Result<PayloadInfo, DecoderError> {
        ...
        } else if l == 0xf8 && header_bytes[1] == 0 {
            Ok(PayloadInfo::new(1, 1))
        } else {
...
impl<'a> Rlp<'a> {
        ...
        'a: 'view,
    {
        ...
        Ok((
            Rlp::new(&bytes[0..found.header_len + found.value_len]),
            offset,
        ))
    }

    pub fn is_null(&self) -> bool {
        self.bytes.is_empty() || (self.bytes[0] == 0xf8 && self.bytes[1] == 0)
    }
