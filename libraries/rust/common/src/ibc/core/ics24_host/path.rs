...
mod tests {
    ...
    fn test_parse_client_paths_fn() {
        let path = "clients/default-0/clientType";
        ...
        let path = "clients/default-0/clientState";
        let components: Vec<&str> = path.split('/').collect();
        ...
        let path = "clients/default-0/consensusStates/15-31";
        let components: Vec<&str> = path.split('/').collect();
    ...
    fn client_connections_path_parses() {
        let path = "clients/default-0/connections";
        let path = Path::from_str(path);
