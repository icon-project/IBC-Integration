...
func CleanDockerSetup(t DockerSetupTestingT, name string) {
	t.Helper()
	cli, err := client.NewClientWithOpts(client.FromEnv)
	if err != nil {
		panic(fmt.Errorf("failed to create docker client: %v", err))
...
func dockerCleanup(t DockerSetupTestingT, cli *client.Client) func() {
	return func() {
		cleanup(cli, t, t.Name())
	}
}

func cleanup(cli *client.Client, t DockerSetupTestingT, name string) {
	...
	cs, err := cli.ContainerList(ctx, types.ContainerListOptions{
		...
		Filters: filters.NewArgs(
			filters.Arg("label", CleanupLabel+"="+name),
		),
	})
	if err != nil {
	...
	pruneVolumesWithRetry(ctx, t, cli, name)
	pruneNetworksWithRetry(ctx, t, cli, name)
}

func pruneVolumesWithRetry(ctx context.Context, t DockerSetupTestingT, cli *client.Client, name string) {
	...
	err := retry.Do(
		func() error {
			res, err := cli.VolumesPrune(ctx, filters.NewArgs(filters.Arg("label", CleanupLabel+"="+name)))
			if err != nil {
...
func pruneNetworksWithRetry(ctx context.Context, t DockerSetupTestingT, cli *client.Client, name string) {
	var deleted []string
	err := retry.Do(
		func() error {
			res, err := cli.NetworksPrune(ctx, filters.NewArgs(filters.Arg("label", CleanupLabel+"="+name)))
			if err != nil {
				if errdefs.IsConflict(err) {
					// Prune is already in progress; try again.
					return err
