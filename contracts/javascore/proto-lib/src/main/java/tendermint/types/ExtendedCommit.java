...
public class ExtendedCommit extends ProtoMessage {
  ...
  private List<ExtendedCommitSig> extendedSignatures = new ArrayList<>();
  ...
  public List<ExtendedCommitSig> getExtendedSignatures() {
    return this.extendedSignatures;
  }

  public void setExtendedSignatures(List<ExtendedCommitSig> extendedSignatures) {
  ...
  public byte[] encode() {
    return ByteUtil.join(
      ...
      Proto.encodeMessageArray(4, this.extendedSignatures));
  }

  public static ExtendedCommit decode(byte[] data) {
    ExtendedCommit obj = new ExtendedCommit();
    int index = 0;
    int order;
    ...
    while (index < length) {
      ...
      switch(order) {
        ...
        case 4: {
            Proto.DecodeResponse<byte[]> resp = Proto.decodeBytes(data, index);
            index = resp.index;
            obj.extendedSignatures.add(ExtendedCommitSig.decode(resp.res));
            break;
        }
      }
    }
