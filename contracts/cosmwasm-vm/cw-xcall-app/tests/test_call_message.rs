...
use cw_common::{ibc_types::IbcHeight, types::Route,types::PPNA};
...
const MOCK_ROUTES:Vec<Route>=vec![];
...
fn send_packet_success() {
    ...
    let route= get_routes(&ctx);
    let result = call_send_call_message(&mut ctx, MOCK_CONTRACT_TO_ADDR,vec![route], vec![1, 2, 3], None);
