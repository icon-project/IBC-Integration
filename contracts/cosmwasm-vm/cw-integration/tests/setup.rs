...
pub fn ibc_core_contract() -> Box<dyn Contract<Empty>> {
    let contract = ContractWrapper::new(
        cw_ibc_core::execute, cw_ibc_core::instantiate, cw_ibc_core::query)
...
pub fn init_light_client(mut ctx:TestContext)->TestContext{
    ...
    let light_client_addr = ctx.app
        ...
        .unwrap();
    ctx.contracts.insert("light_client".to_string(), light_client_addr);
...
pub fn init_ibc_core(mut ctx:TestContext)->TestContext {
    let ibc_core_code_id = ctx.app.store_code(ibc_core_contract());
    let ibc_core_addr = ctx.app
    .instantiate_contract(
        ibc_core_code_id,
        ctx.sender.clone(),
...
pub fn init_xcall_app_contract(mut ctx:TestContext)->TestContext{
    ...
    let xcall_app_contract_addr = ctx.app
    .instantiate_contract(
    ...
    .unwrap();
    ctx.contracts.insert("xcall_app".to_string(), xcall_app_contract_addr);
    ctx

}

pub fn init_xcall_ibc_connection_contract(mut ctx:TestContext){
    let ibc_connection_contract_code_id = ctx.app.store_code(ibc_connection_contract());
    let ibc_connection_contract_addr = ctx.app
    .instantiate_contract(
...
pub fn setup_context() -> TestContext {
    ...
    let sender = Addr::unchecked("sender");
    return TestContext { app: router, contracts: HashMap::new(), sender};
